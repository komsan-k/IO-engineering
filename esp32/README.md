...
```mermaid
flowchart TB
    WN["Wireless Networks<br/>Router • Smartphone • Cloud"]
    PS["Power Supply<br/>USB • Battery • Voltage Regulator"]

    subgraph ESP32["ESP32 System-on-Chip (SoC)"]
        direction TB

        BUS["System Bus"]

        CPU["CPU Core"]
        MEM["ROM • SRAM • Flash Interface"]
        WIFI["Wi-Fi"]
        BLE["Bluetooth / BLE"]
        GPIO["GPIO"]
        ADC["ADC • DAC"]
        PWM["PWM • Touch"]
        COM["UART • I²C • SPI • I²S"]
        TIMER["Timers • RTC • Watchdog"]
        SEC["Security"]
        POWER["Clock & Power Management"]
    ...
    subgraph OUTPUTS["Actuators and Outputs"]
        LED["LED"]
        MOTOR["Motor"]
        RELAY["Relay"]
        SERVO["Servo"]
...
**Figure 1.** Functional block diagram of an ESP32-based intelligent object. The ESP32 integrates processing, memory, wireless communication, peripheral interfaces, security, and power management to connect sensors with actuators and network services.
